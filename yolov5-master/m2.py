...
class ConfusionMatrix:
    ...
    def plot(self, normalize=False, save_dir='', names=()):
        if normalize:
            array = self.matrix / self.matrix.sum(axis=1, keepdims=True)  # 将每行归一化为总数
            fmt = '.2f'  # 使用浮点格式
        else:
        ...
        with warnings.catch_warnings():
            ...
            sn.heatmap(array,
                       ...
                       square=True,
                       vmin=0.0,
                       vmax=1.0 if normalize else None,  # 根据是否归一化设置颜色映射范围
...
# 绘制混淆矩阵，并将值转换为概率
conf_matrix.plot(normalize=True, save_dir='./runs', names=names)
